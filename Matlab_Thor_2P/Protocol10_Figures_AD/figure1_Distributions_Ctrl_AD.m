function figure1_Distributions_Ctrl_AD

adatac = evalin('base','datac');
adataa = evalin('base','dataa');
mData = evalin('base','mData');
colors = {'k','k','k','k','b','b','b','b'};%mData.colors;
...
for ii = 1:4% context
    distDi = [];
    for jj = 1:length(selAnimalsc)
         distDi = [distDi getVariableValues(adatac{selAnimalsc(jj)},'rs',ii)];
    end
    distD{ii} = distDi;
end

for ii = 5:8%length(data)
    distDi = [];
    for jj = 1:length(selAnimalsa)
         distDi = [distDi getVariableValues(adataa{selAnimalsa(jj)},'rs',ii-4)];
    end
    distD{ii} = distDi;
end
...
save_pdf(hf,mData.pdf_folder,'Distribution Of Rs2 Ctrl AD',600);
return;
end
%%
runThis = 1;
...
for ii = 1:4%length(data)
    distDi = [];
    for jj = 1:length(selAnimalsc)
         distDi = [distDi getVariableValues(adatac{selAnimalsc(jj)},'SI',ii)];
    end
    distD{ii} = distDi;
end
...
for ii = 5:8%length(data)
    distDi = [];
    for jj = 1:length(selAnimalsa)
         distDi = [distDi getVariableValues(adataa{selAnimalsa(jj)},'SI',ii-4)];
    end
    distD{ii} = distDi;
end
...
save_pdf(hf,mData.pdf_folder,'Distribution Of zMI Ctrl AD',600);
return;
end
%%
annovaVar = [];
