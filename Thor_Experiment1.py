...
class Thor_Exp:
    ...
    def __init__(self,dir_name,p_dir_name_main,tif_data_folder,nas_p_dir_name_main):
#        print(tif_data_folder)
        self.d_dir = dir_name # data directory name
        self.pd_dir_main = p_dir_name_main #
        self.nas_pd_dir_main = nas_p_dir_name_main #
...
#            return
        ...
        if not tif_data_folder.strip():
            ...
            if self.exp_params.get('zFastEnable') == '1': # check to see if multiple plane data exists
                print(len(self.abf.channel_data.get('frames_f')))
                actual_number_of_frames = len(self.abf.channel_data.get('frames_f'))-1
    #            actual_number_of_frames = 101
                if actual_number_of_frames%2 == 0:
                    actual_number_of_frames = actual_number_of_frames - 1
                num_planes = int(self.exp_params.get('steps'))
                print('Number of Planes is ' + '{}'.format(num_planes))
                self.exp_params.update({'nplanes':num_planes})
                self.exp_params.update({'actual_number_of_frames':actual_number_of_frames})
                self.exp_params['timepoints'] = actual_number_of_frames
                self.exp_params['frames'] = actual_number_of_frames
            else:
...
#         self.__save_processing_status('s2p',1)
    ...
    def __raw_to_tif(self):
        ...
        nas_databin_filename  = self.nas_pd_dir + '/suite2p/plane0/data.bin'
        print(databin_filename)
        print(nas_databin_filename)
        print(self.tif_dir_name)
...
#            actual_number_of_frames = 101
            ...
            self.exp_params['timepoints'] = actual_number_of_frames
            self.exp_params['frames'] = actual_number_of_frames
            if path.exists(databin_filename) or path.exists(nas_databin_filename):
