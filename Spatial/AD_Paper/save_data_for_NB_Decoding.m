...
rast = raster_data_C{1}{1}.fromFrames.sp_rasters(1:2:10,:,:);
rast = permute(rast,[2 1 3]);
rast1 = reshape(rast,size(rast,1)*size(rast,2),size(rast,3));
figure(1);clf;plot(rast1(:,1));

fileName = fullfile(mData.pd_folder,sprintf('NB_decoding_raster_C.mat'));
save(fileName,'raster_data_C','-v7.3');

fileName = fullfile(mData.pd_folder,sprintf('NB_decoding_raster_A.mat'));
save(fileName,'raster_data_A','-v7.3');
return
