...
%         [clus] = getParamValues('cluster4',ei(selAnimals(jj)),planeNumbers,contextNumber(ss),stimMarkers{ss},'dist',selCells,maxDistTime);
        [placeCells] = getParamValues('placeCells2',ei(selAnimals(jj)),planeNumbers,contextNumber(ss),stimMarkers{ss},'dist',selCells,maxDistTime);
    end
    dataC{ss} = tempVals;
end
n = 0;
...
coiSI = find(resp_valsC{selAnimals}(:,conditionNumber))'
% plotRasters_simplest(RsC{selAnimals,conditionNumber},coiSI);
% return
...
cellList = ccsi;
tei.plane{pl}.tP.ops = get_Fall_param(tei.plane{pl}.s2p_folder);
