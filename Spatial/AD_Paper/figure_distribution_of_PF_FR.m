...
for rr = 1:size(RsC,1)
    for cc = 1:size(RsC,2)
        ...
        zMIsC{rr,cc} = MFR(R.resp.vals)';
        R = RsA{rr,cc};
        [rs,MFR,centers,PWs] = get_gauss_fit_parameters(R.gauss_fit_on_mean,R.bin_width);
        zMIsA{rr,cc} = MFR(R.resp.vals)';
    end
end
n = 0;
%%
...
% writetable(dataT,fullfile(mData.pdf_folder,'PFs_MFR.xls'));
%%
...
put_axes_labels(gca,{[],[0 0 0]},{{'Place Field Peak','Firing Rate (AU)'},[0 0 0]});

save_pdf(hf,mData.pdf_folder,'PF_MFR_bar_graph_place_cells.pdf',600);
...
save_pdf(hf,mData.pdf_folder,'PF_MFR_bar_graph_place_cells_pooled.pdf',600);