...
processed_data_folder{3} = '\\mohajerani-nas.uleth.ca\storage\homes\brendan.mcallister\2P\Processed_Data_AD_paper\Matlab_bw3\Control';
...
processed_data_folder{3} = '\\mohajerani-nas.uleth.ca\storage\homes\brendan.mcallister\2P\Processed_Data_AD_paper\Matlab_bw3\APP';
...
for ii = 1:length(ei10_C)
    ei10_C(ii) = make_and_load_rasters(ei10_C(ii),binwidths,nowr,contextIE);
end
for ii = 1:length(ei10_A)
    ei10_A(ii) = make_and_load_rasters(ei10_A(ii),binwidths,nowr,contextIE);
end
