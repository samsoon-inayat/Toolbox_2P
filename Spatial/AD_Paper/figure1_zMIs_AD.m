...
% cellsOrNot = 1; planeNumber = NaN; zMI_Th =1.96; fwids = [1 120]; fcens = [0 140]; rs_th = 0.3;
...
if 1
    dataT = [[ones(size(var_oi_C,1),1);2*ones(size(var_oi_A,1),1)] [var_oi_C;var_oi_A]];
    between = array2table(dataT);
    between.Properties.VariableNames = {'Group','C1','C2','C3','C4'};
    between.Group = categorical(between.Group);
    within = array2table([1 2 3 4]'); within.Properties.VariableNames = {'Conds'};
    within.Conds = categorical(within.Conds);
    ra = repeatedMeasuresAnova(between,within,0.05);
end
%%
runthis = 1;
if runthis
