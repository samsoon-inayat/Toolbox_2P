...
for rr = 1:size(pMs_C,1)
    for cc = 1:size(pMs_C,2)
        ...
        for an = 1:length(selAnimals_C)
            zMIs_C(an,rr,cc) = nanmean(squeeze(pMs_C{rr,cc}.all_fwidths{selAnimals_C(an)}(nds(1),nds(2),:)));
            a_zMIs_C{an,rr,cc} = squeeze(pMs_C{rr,cc}.all_fwidths{selAnimals_C(an)}(nds(1),nds(2),:));
...
for rr = 1:size(pMs_A,1)
    for cc = 1:size(pMs_A,2)
        ...
        for an = 1:length(selAnimals_A)
            zMIs_A(an,rr,cc) = nanmean(squeeze(pMs_A{rr,cc}.all_fwidths{selAnimals_A(an)}(nds(1),nds(2),:)));
            a_zMIs_A{an,rr,cc} = squeeze(pMs_A{rr,cc}.all_fwidths{selAnimals_A(an)}(nds(1),nds(2),:));
...
%     xtickangle(30)
    ...
    put_axes_labels(gca,{[],[0 0 0]},{'Field Widths',[0 0 0]});
