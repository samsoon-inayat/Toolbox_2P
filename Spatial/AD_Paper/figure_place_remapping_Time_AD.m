function figure_place_remapping_Time_AD(fn,allRs,ccs)
...
included_cells = 'All';
conditionsAndRasterTypes = [12;22;32;42];
...
if 1
    ...
    writetable(between,fullfile(mData.pdf_folder,sprintf('%s_dataT_%s.xlsx','figure_place_remapping_Time_AD',included_cells)));
    writetable(rmaR.ranova,fullfile(mData.pdf_folder,sprintf('%s_dataT_stats_output_%s.xlsx','figure_place_remapping_Time_AD',included_cells)),'WriteRowNames',true);
...
%     findMeanAndStandardError
    ...
    save_pdf(hf,mData.pdf_folder,sprintf('%s_bargraph_overall_%s','figure_place_remapping_Time_AD',included_cells),600);
end
n = 0;
...
%     xlim([-3 12])
    ...
    file_name = sprintf('%s_distribution CC_%s_%d','figure_place_remapping_Time_AD',included_cells,cond);
...
for rr = 1:size(pMs_C,1)
    for cc = 1:size(pMs_C,2)
        ...
        for an = 1:length(selAnimals_C)
            ...
            stimMarker = paramMs_C.stimMarkers{nds(2)}; maxDistTime = [140 5];%paramMs_C.maxDistTime;
