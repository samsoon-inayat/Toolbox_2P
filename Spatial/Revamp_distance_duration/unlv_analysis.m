...
mFR = []; mSP = []; mAM = [];
for ani = 1:5
    tempv = get_unlv_analysis_mFR_stim_combs(udata{ani});
    mFR(ani,:) = tempv.FR;     mSP(ani,:) = tempv.SP;     mAM(ani,:) = tempv.AM;
end
...
for ani = 1:5
    ...
    msaonoff = [msaonoff;[mean_speed_air_on(1) mean_speed_air_off(1) mean_speed_air_on(2) mean_speed_air_off(2) mean_speed_air_on(3) mean_speed_air_off(3)]];
...
%%
[within,dvn,xlabels] = make_within_table({'Conf','APhase'},[3,2]);
...
%%
magfac = mData.magfac;
ff = makeFigureRowsCols(107,[3 5 2.2 1],'RowsCols',[1 2],'spaceRowsCols',[0.01 -0.02],'rightUpShifts',[0.07 0.3],...
    'widthHeightAdjustment',[10 -450]);
MY = 40; ysp = 9; mY = 0; ystf = 7; ysigf = 1;titletxt = ''; ylabeltxt = {'PDF'}; % for all cells (vals) MY = 80
stp = 0.33*magfac; widths = [1.1 0.35 2.85 1]*magfac+0.061; gap = 0.09105*magfac;
adjust_axes(ff,[mY MY],stp,widths,gap,{''});
...
tcolors = repmat(mData.colors(5:end),1,6);
[hbs,xdata] = view_results_rmanova(ff.h_axes(1,1),ra,{'Conf:APhase','hsd',0.05,1},xs_gaps,tcolors,[mY MY ysp ystf ysigf],mData);
xticklabels = {'AOn','AOff'}; set(gca,'xtick',xdata,'xticklabels',xticklabels); xtickangle(15);
ylabel('Avg. Speed (cm/s)');
make_bars_hollow(hbs(2:2:end));
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,2,{'C3','C4','C5'},{[0.001 0.0051]});

tcolors = repmat(mData.colors(1:end),1,6);
[hbs,xdata] = view_results_rmanova(ff.h_axes(1,2),ra,{'APhase','hsd',0.05},xs_gaps,tcolors,[mY MY ysp ystf ysigf],mData);
xticklabels = {'AOn','AOff'}; set(gca,'xtick',xdata,'xticklabels',xticklabels); xtickangle(30);
