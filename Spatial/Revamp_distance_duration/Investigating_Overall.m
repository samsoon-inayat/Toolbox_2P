        ...
        for cn = 1:length(configurations)
            for ap = 1:length(air_phases)
                ...
                for vn = 1:length(variable_combs)
                    ...
                    zMI_vals = [zMI_vals MIs(1,2)]; zPC_vals = [zPC_vals PCs(1,2)];
                end
            end
        end
    end
...
%% my analysis about the no-brake condition of what I am learning in terms 
% of critical thinking of what are some of the things that need to be 
% checked when you are analyzing the data

for an = 1:5
    data_an = udataT{an};
    firing_rate = data_an.firing_rate;
    size(firing_rate);
    num_cells(an) = size(firing_rate,1);
    % mFR = nanmean(firing_rate,2);
    omFR(an) = nanmean(firing_rate,"all");
    % figure(100);clf;
    % imagesc(firing_rate);colorbar
    % histogram(firing_rate(:),'Normalization','probability')
    % pause
end
