...
%% Collecting the zMIs all in one big matrix for each animal across configurations, phases
...
fidelity_threshold = 0.3;  % Example threshold, adjust as needed
...
[within,dvn,xlabels,awithinD] = make_within_table({'time_dist','Configurations','Phases'},[2,3,2]);
dataT = make_between_table({[mean_filtered_zMI_time mean_filtered_zMI_dist]},dvn);
ra = RMA(dataT,within,{0.05,{'hsd'}});
ra.ranova
print_for_manuscript(ra)

[EMs,GS] = RMA_get_EM_GS(ra.rm,{'time_dist'})

%% looking at the changes in time- and distance- dominance across phases
% Define stability threshold (e.g., neurons with less than this difference in zMI are stable)
stability_threshold = 0.1;  % Adjust as needed

% Initialize matrices for stable neurons across phases
stable_time_neurons = cell(size(zMI_values));
stable_distance_neurons = cell(size(zMI_values));

% Loop over animals, configurations, and neurons to check zMI stability across phases
for ani_idx = 1:num_animals
    for config_idx = 1:num_configs
        % Extract zMI for time and distance in both air-on and air-off phases
        time_zMI_on = zMI_values{ani_idx, (config_idx - 1) * 2 + 1};  % Air-on time zMI
        time_zMI_off = zMI_values{ani_idx, (config_idx - 1) * 2 + 2};  % Air-off time zMI
        dist_zMI_on = zMI_values{ani_idx, 6 + (config_idx - 1) * 2 + 1};  % Air-on distance zMI
        dist_zMI_off = zMI_values{ani_idx, 6 + (config_idx - 1) * 2 + 2};  % Air-off distance zMI

        % Check stability for time encoding (difference between air-on and air-off phases)
        stable_time_neurons{ani_idx, config_idx} = abs(time_zMI_on - time_zMI_off) < stability_threshold;

        % Check stability for distance encoding (difference between air-on and air-off phases)
        stable_distance_neurons{ani_idx, config_idx} = abs(dist_zMI_on - dist_zMI_off) < stability_threshold;
    end
end

% Calculate percentage of stable neurons for time and distance across configurations
percentage_stable_time_neurons = cellfun(@(stable, zMI) 100 * sum(stable) / size(zMI, 1), stable_time_neurons, zMI_values);
percentage_stable_distance_neurons = cellfun(@(stable, zMI) 100 * sum(stable) / size(zMI, 1), stable_distance_neurons, zMI_values);

% Visualize the percentage of stable neurons across configurations
figure(1000); clf;
bar([percentage_stable_time_neurons; percentage_stable_distance_neurons]');
legend('Stable Time Neurons (%)', 'Stable Distance Neurons (%)');
xlabel('Configurations');
ylabel('Percentage of Stable Neurons (%)');
title('Percentage of Stable Neurons Across Phases (Time vs. Distance)');
