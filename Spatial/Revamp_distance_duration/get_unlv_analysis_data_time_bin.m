...
time_bins = 0:time_bin_size:max(ts);  % Time bins from 0 to max time in ts variable
frf = tudata.frf;
firing_rate = tudata.firing_rate;
...
bin_indices = discretize(ts, time_bins);  % bin_indices maps each ts value to a time bin
...
frame_times = ts(frf);  % Get the actual frame times corresponding to 'frf'
