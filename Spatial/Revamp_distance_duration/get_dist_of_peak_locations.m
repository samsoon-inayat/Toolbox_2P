function [dist, distL, distTr, distTrL] = get_dist_of_peak_locations(cellpop,si,mRs,allpeakL_trials,allresp_trials,binEs)
...
for cn = 1:length(si)
    for an = 1:5
        pLs = allpeakL_trials{an,cn}; resp = allresp_trials{an,cn};
        trNi = 1;
        for trN = trials
            switch rccc
                case 1
