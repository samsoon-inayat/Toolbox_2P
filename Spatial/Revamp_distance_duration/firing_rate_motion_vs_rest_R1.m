...
Rs_C_MC = o.RsMC(:,si); mRs_C_MC = o.mR1(:,si); 
...
for sii = 1:length(si)
    ...
    for ani = 1:5
        ...
        avgFR_animal(ani,1) = nanmean(z_firing_rate,'All')
    end
    all_avgFR{sii} = avgFR;
    mall_avgFR{sii} = mean(avgFR);
    MFR = [MFR max(mall_avgFR{sii})];  mFR = [mFR min(mall_avgFR{sii})];
    xsFR{sii} = tR.xs;
    all_avgFR_animals(:,sii) = avgFR_animal;
...
xlabelsT = {'s','s','cm','s','cm','s','cm','s','s','s'};
aoo = {'Air-On','Air-Off','Air-On','Air-Off','Air-On','Air-Off','Air-On','Air-Off','Air-On','Air-Off'};
...
magfac = mData.magfac;
ff = makeFigureRowsCols(109,[3 3 4.8 1.5],'RowsCols',[1 10],'spaceRowsCols',[0.01 -0.02],...
...
set_sub_graph_text(ff,2,{'C2','C3','C4','C5','C7'},[0 -0.2 0 0],[0.0 0.0 0 0]);
...
for sii = 1:length(si)
    ...
    for ani = 1:5
        tR = Rs_C{ani,sii};
        firing_rate = tR.sp_rastersZ(:,1:(size(tR.sp_rasters,2)-1),:);
        avgFR(ani,:) = (nanmean(squeeze(nanmean(firing_rate,1)),2))';
        avgFR_animal(ani,1) = nanmean(firing_rate,'All')
    end
    all_avgFR{sii} = avgFR;
    mall_avgFR{sii} = mean(avgFR);
    MFR = [MFR max(mall_avgFR{sii})];  mFR = [mFR min(mall_avgFR{sii})];
...
xticklabels = {'AOn','AOff'};set(gca,'xtick',xticks,'xticklabels',xticklabels); xtickangle(30);
make_bars_hollow(hbs(2:2:end));
put_axes_labels(gca,{'',[]},{ylabeltxt,[0 -0.01 0]});
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,2,{'C2','C3','C4','C5','C7'},{[-0.02 0.01]});
% ht = set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,8,{'C-TG','A-TG'},{[-0.14 -0]}); 
format_axes(gca);
...
% xdata = make_xdata([1 1 1 1 1],[1 1.5]); 
[hbs,maxY] = plotBarsWithSigLines(mVar,semVar,combs,[h p],'colors',tcolors,'sigColor','k',...
'ySpacing',ysp,'sigTestName','','sigLineWidth',0.25,'BaseValue',0.01,...
'xdata',xdata,'sigFontSize',7,'sigAsteriskFontSize',7,'barWidth',0.5,'sigLinesStartYFactor',0,...
'sigAsteriskyshift',0);
set_axes_limits(gca,[0.35 xdata(end)+.65],[mY MY]); format_axes_b(gca); xticks = xdata; 
xticklabels = {'C2','C3','C4','C5','C7'};set(gca,'xtick',xticks,'xticklabels',xticklabels); xtickangle(30);
% make_bars_hollow(hbs(2:2:end));
% put_axes_labels(gca,{'',[]},{ylabeltxt,[0 -0.01 0]});
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,5,{'Pooled'},{[-0.03 0.01]});
% ht = set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,8,{'C-TG','A-TG'},{[-0.14 -0]}); 
format_axes(gca);
...
[hbs,maxY] = plotBarsWithSigLines(mVar,semVar,combs,[h p],'colors',tcolors,'sigColor','k',...
    'ySpacing',ysp,'sigTestName','','sigLineWidth',0.25,'BaseValue',0.01,...
    'xdata',xdata,'sigFontSize',7,'sigAsteriskFontSize',7,'barWidth',0.45,'sigLinesStartYFactor',0.012,...
    'sigAsteriskyshift',0.0051);
set_axes_limits(gca,[0.35 xdata(end)+.65],[mY MY]); format_axes_b(gca); xticks = xdata; 
xticklabels = {'AOn','AOff'};set(gca,'xtick',xticks,'xticklabels',xticklabels); xtickangle(30);
make_bars_hollow(hbs(2));
ht = set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,2,{'Pooled'},{[-0.01 -0]}); 
format_axes(gca);
