...
for sii = 1:length(si)
    ...
    for ani = 1:5
        tR = Rs_C{ani,sii};
        avgFR(ani,:) = (nanmean(squeeze(nanmean(tR.sp_rasters1,1)),2))';
    end
    all_avgFR{sii} = avgFR;
...
magfac = mData.magfac;
ff = makeFigureRowsCols(108,[3 3 4.8 1],'RowsCols',[1 10],'spaceRowsCols',[0.01 -0.02],...
    'rightUpShifts',[0.31 0.4],'widthHeightAdjustment',[10 -650]);
MY = 0.15; ysp = 0.025; mY = 0; titletxt = ''; ylabeltxt = {'Avg. FR (A.U.)'};
...
%     shadedErrorBar(xsFR{gii},m_avgFR,sem_avgFR,{'color',colors{gii},'linewidth',0.25},0.7);
    xlim([1 length(m_avgFR)]);ylim([0 0.3]);
    if gii > 1
        set(gca,'YTick',[]);
    else
        ylabel(ylabeltxt);
...
%%
magfac = mData.magfac;
ff = makeFigureRowsCols(108,[10 3 2.8 1],'RowsCols',[1 2],'spaceRowsCols',[0.01 -0.02],'rightUpShifts',[0.31 0.40],'widthHeightAdjustment',[10 -450]);
MY = 0.2; ysp = 0.02; mY = 0; titletxt = ''; ylabeltxt = {'Avg. FR (A.U.)'};
stp = 0.3*magfac; widths = ([2 0.4 0.4 1.3 1.3 0.5 0.5 0.5])*magfac; gap = 0.051*magfac;
adjust_axes(ff,[mY MY],stp,widths,gap,{''});
colors = mData.colors;
