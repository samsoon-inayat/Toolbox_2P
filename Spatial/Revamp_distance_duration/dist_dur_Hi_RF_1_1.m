...
while 1
    ...
    per_active_all = find_percent(any_cells);
...
while 1
    ...
    mean_dzMI = [ mean_dzMI get_vals(TD,cell_resp)];
...
%% compare the values of zMI for the four raster types
while 1
    rfi = 4;
    tprops = props(rfi,:);
    mean_dzMI = [];
    for ii = 1:length(tprops)
        ttprops = tprops{ii};
        mean_dzMI = [mean_dzMI exec_fun_on_cell_mat(ttprops.zMI,'nanmean',ttprops.good_FR)];
    end
...
%% compare the values of dzMI across trials and intertrials
while 1
    rfi = 4;
...
%     h(h==1) = 0;
    ...
    changePosition(gca,[0.04 0.01 -0.55 0]); put_axes_labels(gca,{[],[0 0 0]},{'Cells (%)',[0 0 0]});
    save_pdf(hf,mData.pdf_folder,sprintf('perc_cells_all.pdf'),600);
    %%
    break
end
...
%% Overlap Indices ImageSC checking only the three conditions and dis and dur cells
while 1
    %%
%     si = [Ar_On ArL_On Ars_On]; props_ON = get_props_Rs(o.Rs(:,si)); resp_ON = cell_list_op(props_ON.vals,[],'or',1);
%     si = [Ar_Off ArL_Off Ars_Off]; props_OFF = get_props_Rs(o.Rs(:,si)); resp_OFF = cell_list_op(props_OFF.vals,[],'or',1);
