...
xticklabels = {'Tr','Dr'};set(gca,'xtick',xticks,'xticklabels',xticklabels);% xtickangle(30);
...
switch varT
    ...
    case 3
        MY = 0.1; ysp = 0.01; mY = -0.15; titletxt = 'Mutual Information'; ylabeltxt = {'Mutual Information','Z-Score'};
        MY = 3; ysp = 0.3; mY = 0; titletxt = 'Mutual Information'; ylabeltxt = {'z-score'};
    case 9
        MY = 20; ysp = 1; mY = -0.15; titletxt = 'Tuning Width'; ylabeltxt = {'cm'};
end
...
ht = set_axes_top_text_no_line(gcf,gca,'Responsivity',[-0.05 -0.01 0.2 0]);set(ht,'FontWeight','Bold');
...
%     hf = get_figure(5,[8 7 1.5 1]);
tcolors = repmat(mData.colors(1:3),3,1);
MmVar = max(mVar);
[hbs,maxY] = plotBarsWithSigLines(mVar,semVar,combs,[h p],'colors',tcolors,'sigColor','k',...
    'ySpacing',MmVar/1.95,'sigTestName','','sigLineWidth',0.25,'BaseValue',0.01,...
    'xdata',xdata,'sigFontSize',7,'sigAsteriskFontSize',8,'barWidth',0.5,'sigLinesStartYFactor',0.15);
...
%     hf = get_figure(5,[8 7 1.5 1]);
    ...
    xticks = xdata; xticklabels = {'Air','No Air'};
    set(gca,'xtick',xticks,'xticklabels',xticklabels); xtickangle(45)
    put_axes_labels(gca,{[],[0 0 0]},{'% of trials',[0 0 0]});
