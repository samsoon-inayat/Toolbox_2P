...
stp = 0.225*magfac; widths = ([1.5 0.55 1.3 1.3 1.3 0.5 0.5 0.5]-0.05)*magfac; gap = 0.067*magfac;
...
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,6,{'Trials','InterTrials'},{[-0.1 -0.012]});
...
switch varT
    case 1 % responsive cells 
        MY = 70; ysp = 4; mY = 0; titletxt = 'Responsivity'; ylabeltxt = {'% of cells'}; % for all cells (vals) MY = 80
    case 2
        MY = 70; ysp = 4; mY = 0; titletxt = 'Response Fidelity'; ylabeltxt = {'% of Trials'};% for all cells (vals) MY = 70
    case 3
        MY = 0.1; ysp = 0.01; mY = -0.15; titletxt = 'Mutual Information'; ylabeltxt = {'Mutual Information','Z-Score'};
        MY = 3.5; ysp = 0.3; mY = 0; titletxt = 'Mutual Information'; ylabeltxt = {'(z-score)'};
    case 9
...
ht = set_axes_top_text_no_line(gcf,gca,titletxt,[-0.051 -0.051 0.21 0]);set(ht,'FontWeight','Bold');
format_axes(gca);
save_pdf(ff.hf,mData.pdf_folder,'bar_graph.pdf',600);
...
break;
 ...
 while 1
    ...
    format_axes(gca);
    set_axes_limits(gca,[0.35 xdata(end)+.65],[ylims(1) maxY]); format_axes(gca);
    xticks = xdata; xticklabels = {'D','T','Mix'};
    set(gca,'xtick',xticks,'xticklabels',xticklabels); xtickangle(45)
