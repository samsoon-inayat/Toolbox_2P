...
hf = figure(1000);clf;set(gcf,'Units','Inches');set(gcf,'Position',[15 5 4.5 2.5],'color','w');
hold on;
xdatas = {[1 2 3],[5 6 7]};
maxY = 0.27;
for ii = 1:2
    ...
    plotBarsWithSigLines(sigR.means,sigR.sems,sigR.combs,[hs ps],'colors',colors,'sigColor',sigColor,...
        'maxY',maxY,'ySpacing',0.1,'sigTestName','ANOVA','sigLineWidth',0.25,'BaseValue',0,...
...
for ii = 1:2
    ...
    hs = sigR.anova.multcompare.h; ps = sigR.anova.multcompare.p;
    plotBarsWithSigLines(sigR.means,sigR.sems,sigR.combs,[hs ps],'colors',colors,'sigColor',sigColor,...
        'maxY',0.27,'ySpacing',0.1,'sigTestName','ANOVA','sigLineWidth',0.25,'BaseValue',0,...
        'xdata',xdatas{ii},'sigFontSize',7);
end
hx = xlabel('Exc/Inh/Context'); hy = ylabel('Percentage of Cells');
xlim([0 16]);ylim([0 maxY]);
legs = {'Context 1','Context 2','Context 3',[1 0.25 0.25 0.025]};
putLegend(gca,legs,'colors',colors,'sigR',{[],'ks',sigColor,10});
set(gca,'XTick',[2 6 10 14],'XTickLabel',{'Pre-Exc','Post-Exc','Pre-Inh','Post-Inh'});
set(gca,'FontSize',mData.axes_font_size,'FontWeight','Bold','TickDir','out');changePosition(gca,[-0.03 0 0.1 -0.05]);
text(1,0.29,'Air Puff Responsive and Place Cells','FontSize',mData.axes_font_size,'FontWeight','Bold');
save_pdf(hf,mData.pdf_folder,'Perc Of air and Place Cells_15',600);
return;
end
...
save_pdf(hf,mData.pdf_folder,'Perc Of air Cells_15',600);
return;
end
