...
hold on;
[ha,hb,hca,sigR] = plotDistributions(distD,'colors',colors,'maxY',40,'cumPos',[0.5 0.36 0.25 0.3],'min',0,'incr',0.05,'max',1);
hold on;
legs = [];
for ii = 1:length(distD)
    legs{ii} = sprintf('C%d (N = %d)',ii,length(distD{ii}));
end
legs{ii+1} = [0.33 0.03 30 3];
putLegend(ha,legs,'colors',colors,'sigR',{sigR,'anova',sigColor,8});
axes(ha);h = xlabel('Goodness of fit R^2');set(gca,'FontSize',axes_font_size,'FontWeight','Bold');
changePosition(h,[0 -2 0]);h = ylabel('Percentage');changePosition(h,[0.0 0 0]);
axes(hca);set(gca,'FontSize',6);
set(gca,'FontSize',axes_font_size,'FontWeight','Bold');changePosition(ha,[-0.04 0.09 0.12 -0.06]);
save_pdf(hf,mData.pdf_folder,'Distribution Of Rs2_15',600);
return;
end
%%
runThis = 1;
...
save_pdf(hf,mData.pdf_folder,'Distribution Of zMI_15',600);
return;
end
%%
annovaVar = [];
