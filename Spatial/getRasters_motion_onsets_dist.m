function allA_P = getRasters_motion_onsets_dist(ei,trials,ow)
...
if iscell(ei)
    for ii = 1:length(ei)
        temp = getRasters_motion_onsets_dist(ei{ii},trials,ow);
        if ii == 1
...
a_p_onsets = motionOnsets - round(1e6 * timeBefore/ei.b.si);
a_p_offsets = motionOnsets + round(1e6 * timeAfter/ei.b.si);
