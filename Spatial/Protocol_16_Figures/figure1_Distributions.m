...
hold on;
[ha,hb,hca,sigR] = plotDistributions(distD,'colors',colors,'maxY',40,'cumPos',[0.5 0.36 0.25 0.4],'min',0,'incr',0.05,'max',1);
hold on;
legs = [];
for ii = 1:length(distD)
    legs{ii} = sprintf('C%d (N = %d)',ii,length(distD{ii}));
end
legs{ii+1} = [0.3 0.03 40 3.5];
...
save_pdf(hf,mData.pdf_folder,'Distribution Of Rs2_16',600);
return;
end
%%
runThis = 1;
...
hold on;
[ha,hb,hca,sigR] = plotDistributions(distD,'colors',colors,'maxY',60,'cumPos',[0.5 0.36 0.25 0.3],'min',0,'incr',1,'max',21);
hold on;
legs = [];
for ii = 1:length(distD)
    legs{ii} = sprintf('C%d (N = %d)',ii,length(distD{ii}));
end
legs{ii+1} = [5 0.3 60 4.3];
...
save_pdf(hf,mData.pdf_folder,'Distribution Of zMI_16',600);
return;
end
%%
annovaVar = [];
