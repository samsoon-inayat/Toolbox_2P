function figure3_populationVectors_dynamics(fn,allRs,ccs)

data = evalin('base','data');
mData = evalin('base','mData');
colors = mData.colors;
sigColor = mData.sigColor;
n = 0;
%%
trials = 3:10;
trialsC = {[1:2],[3:4],[5:6],[7:10]};
ii = 4; context = ii;
selCells = selectCells(data,mData,'Context',ii);
% selCells = selectCells(data,mData,'Common',[1 2]);
dataForAlignment = data{ii};
[P,C,cellNums] = findPopulationVectorPlot(dataForAlignment,selCells,trials);
allPeakPos = []; allP = []; allPR = []; allC = [];
for sii = 1:4
    allRs = data(ii);
    Rs = allRs{1};
    trials = trialsC{sii};
    [P,C] = findPopulationVectorPlot(Rs,selCells,trials,cellNums);
    allP{sii} = P;
    allC{sii} = C;
    minC(sii) = min(C(:));
    [~,temp] = max(P,[],2);
    allPeakPos(:,sii) = Rs.dist(temp);
    temp = findMeanRasters(Rs,trials);
    temp = temp(selCells,:);
    allPR(:,:,sii) = temp(cellNums,:);
end
n = 0;

%%
ff = makeFigureRowsCols(105,[25 0.5 4 1],'RowsCols',[2 4],...
    'spaceRowsCols',[-0.06 -0.065],'rightUpShifts',[0.1 0.04],'widthHeightAdjustment',...
    [30 -15]);
gg = 1;
set(gcf,'color','w');
set(gcf,'Position',[15 6 3.5 2]);
FS = 5;
for sii = 1:4
    axes(ff.h_axes(1,sii));changePosition(gca,[0 0.05 -0.091 -0.1]);
    imagesc(allP{sii});
    axis off
    if sii == 1
        text(-5,1,'1','FontSize',FS,'FontWeight','Normal');
        if size(P,1) < 100
            text(-7,size(P,1),sprintf('%d',size(P,1)),'FontSize',FS,'FontWeight','Normal');
        else
            text(-10,size(P,1),sprintf('%d',size(P,1)),'FontSize',FS,'FontWeight','Normal');
        end
        text(-21,25,sprintf('Cells'),'FontSize',FS+3,'FontWeight','Bold','rotation',90);
    end
    trials = trialsC{sii};
    text(3,size(P,1)+round(size(P,1)/10),sprintf('Trials %d-%d',trials(1),trials(end)),'FontSize',FS,'FontWeight','Normal');
    set(gca,'Ydir','Normal','linewidth',1.5,'FontSize',FS,'FontWeight','Bold','YTick',[1 length(selCells)]);
    cols = size(Rs.rasters(:,:,1),2);
    colsHalf = ceil(cols/2);
    ts = round(Rs.dist);
    set(gca,'XTick',[1 colsHalf cols],'XTickLabel',[ts(1) ts(colsHalf) ts(cols)]);
    set(gca,'XTick',[]);
    axes(ff.h_axes(2,sii));
    dec = -0.09;
    changePosition(gca,[0.0 0.05 dec dec]);
    imagesc(allC{sii},[-1 1]);
    box off;
    axis equal
    axis off
    if sii == 1        
        text(-8,3,'0','FontSize',FS,'FontWeight','Normal');
        text(-10,50,'142','FontSize',FS,'FontWeight','Normal');
    end
    text(-1,-3,'0','FontSize',FS,'FontWeight','Normal');
    text(44,-3,'142','FontSize',FS,'FontWeight','Normal');
    if sii == 2
        text(35,-13,sprintf('Position (cm)'),'FontSize',FS+3,'FontWeight','Bold','rotation',0);
    end
    if sii == 1
        text(-21,-3,sprintf('Position (cm)'),'FontSize',FS+3,'FontWeight','Bold','rotation',90);
    end
    set(gca,'Ydir','Normal','linewidth',1,'FontSize',FS,'FontWeight','Bold');
    h = xlabel('Position (cm)');
    changePosition(h,[0 0 0]);
    h = ylabel('Position (cm)');
    changePosition(h,[1 0 0]);
    cols = size(Rs.rasters(:,:,1),2);
    colsHalf = ceil(cols/2);
    ts = round(Rs.dist);
    set(gca,'XTick',[1 colsHalf cols],'XTickLabel',[ts(1) ts(colsHalf) ts(cols)]);
    set(gca,'YTick',[1 colsHalf cols],'YTickLabel',[ts(1) ts(colsHalf) ts(cols)],'Ydir','Normal');
end

colormap parula
mI = min(minC);
for ii = 1:4
    axes(ff.h_axes(2,ii));
    caxis([mI 1]);
end

axes(ff.h_axes(2,4));
pos = get(gca,'Position');
h = axes('Position',pos);
changePosition(h,[0.14 0.06 0.0 -0.1]);
hc = colorbar; axis off
set(hc,'YTick',[],'linewidth',0.01);
changePosition(hc,[0 0 0 -0.01]);
ylims = ylim;
xlims = xlim;
text(xlims(1)+0.33,ylims(1)-0.05,sprintf('%.2f',mI),'FontSize',4.5);
text(xlims(1)+0.39,ylims(2)+0.045,'1','FontSize',4.5);

axes(ff.h_axes(1,4));
pos = get(gca,'Position');
h = axes('Position',pos);
changePosition(h,[0.14 0.06 0.0 -0.1]);
hc = colorbar; axis off
set(hc,'YTick',[],'linewidth',0.01);
changePosition(hc,[0 0 0 -0.01]);
ylims = ylim;
xlims = xlim;
text(xlims(1)+0.4,ylims(1)-0.05,sprintf('0'),'FontSize',4.5);
text(xlims(1)+0.39,ylims(2)+0.045,'1','FontSize',4.5);

save2pdf('figure_pf_dynamics_trials.pdf',ff.hf,600);

%%
% bar graph from anova
sigR = significanceTesting(allP);
ff = makeFigureWindow__one_axes_only(100,[2 4 2 2],[0.3 0.22 0.68 0.72]);
set(gcf,'color','w');
set(gcf,'Position',[25 5 1.2 1.5]);
axes(ff.ha); hs = sigR.anova.multcompare.h; ps = sigR.anova.multcompare.p;
plotBarsWithSigLines(sigR.means,sigR.sems,sigR.combs,[hs ps],'colors',colors,'sigColor',sigColor,'maxY',0.25,'ySpacing',0.02,'sigTestName','ANOVA','BaseValue',0);
% plotBarsWithSigLines(sigR.means,sigR.sems,sigR.combs,[hs ps],'colors',colors,'sigColor',sigColor,'maxY',0.57,'ySpacing',0.04,'sigTestName','ANOVA');
xlim([0.4 0.6+length(sigR.means)]);
%     ylim([0 max(mVals)]);
hyl = ylabel('Average Normalized FR');
pos = get(hyl,'Position');pos = pos + [+0.4 0 0];set(hyl,'Position',pos);
% set(ff.ha,'linewidth',1);
set(ff.ha,'TickDir','out','FontSize',7,'FontWeight','Normal');
set(ff.ha,'XTick',[1 2 3 4],'XTickLabel',{'Trials 1-2','Trials 3-4','Trials 5-6','Trials 7-10'});
xtickangle(25);
save2pdf('figure_pf_dynamics_trials_bars.pdf',ff.hf,600);
%% peaks shifts

ff = makeFigureWindow__one_axes_only(106,[22 6 2 1.5],[0.19 0.22 0.76 0.72]);
set(gcf,'color','w');
axes(ff.ha);hold on;
distD = [];
distD{1} = allPeakPos(:,1)'-allPeakPos(:,2)';
% distD{2} = allPeakPos(:,1)'-allPeakPos(:,3)';
distD{2} = allPeakPos(:,3)'-allPeakPos(:,4)';
barXics = {'barPos',[0.6 0.25 0.15 0.3],'maxY',30,'ySpacing',3,'sigTestName','t test','sigColor',sigColor};
[hb,hca,sigR] = plotDistributions(distD,'min',-140,'incr',20,'max',140,'maxY',85,'colors',colors,...
    'barGraph',barXics,'BaseValue',0.3,'ks_tail','smaller');
legs = {'Trials (1-2) - Trials (3-4)','Trials (5-6) - Trials (7-10)',[-50 15 85 7]};
putLegend(ff.ha,legs,'colors',colors,'sigR',{sigR,'ks',sigColor,3});
h = xlabel('Difference Peak Position (cm)');
changePosition(h,[0 0.5 0]);
h = ylabel('Percentage');
changePosition(h,[5 0 0]);
axes(hca); 
set(hca,'XTick',[]);ylabel('Mean Difference');
fileName = sprintf('figure_pf_dynamics_peak_shifts_%d.pdf',context)
save2pdf(fileName,ff.hf,600);


%%
trials = 3:10;
trialsC = {[1:2],[3:4],[5:6],[7:10]};
% selCells = selectCells(data,mData,'Common',[1 2]);
allPeakPos = []; allP = []; allPR = []; allC = [];
for ii = 1:4
    selCells = selectCells(data,mData,'Context',ii);
    allRs = data(ii);
    Rs = allRs{1};
    allPeakPos = []; allP = []; 
    dataForAlignment = data{ii};
    [P,C,cellNums] = findPopulationVectorPlot(dataForAlignment,selCells,trials);
    for sii = 1:2
        trials = trialsC{sii};
        [P,C] = findPopulationVectorPlot(Rs,selCells,trials,cellNums);
        allP{sii} = P;
        allC{sii} = C;
        minC(sii) = min(C(:));
        [~,temp] = max(P,[],2);
        allPeakPos(:,sii) = Rs.dist(temp);
    end
    dAllPeakPos{ii} = allPeakPos(:,1)'-allPeakPos(:,2)';
end

ff = makeFigureWindow__one_axes_only(106,[22 6 2 1.5],[0.19 0.22 0.76 0.72]);
set(gcf,'color','w');
axes(ff.ha);hold on;
distD = [];
distD = dAllPeakPos;
barXics = {'barPos',[0.6 0.2 0.15 0.35],'maxY',30,'ySpacing',3,'sigTestName','ANOVA','sigColor',sigColor,'BaseValue',0.4};
[hb,hca,sigR] = plotDistributions(distD,'min',-140,'incr',20,'max',140,'maxY',100,'colors',colors,...
    'barGraph',barXics,'BaseValue',0.3,'ks_tail','smaller');
legs = {'Context 1','Context 2','Context 3','Context 4',[-50 15 100 7]};
putLegend(ff.ha,legs,'colors',colors,'sigR',{sigR,'ks',sigColor,3});
h = xlabel('Difference Peak Position (cm)');
changePosition(h,[0 0.5 0]);
h = ylabel('Percentage');
changePosition(h,[5 0 0]);
axes(hca); 
set(hca,'XTick',[]);h = ylabel('Mean Difference');changePosition(h,[0.5 0 0]);
fileName = sprintf('figure_pf_dynamics_peak_shifts.pdf')
save2pdf(fileName,ff.hf,600);