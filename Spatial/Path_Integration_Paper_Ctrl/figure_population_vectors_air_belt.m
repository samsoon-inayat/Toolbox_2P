...
if 1
    ...
    for ii = 1:length(a_trials)
        ...
        [out.allP_an,out.allC_an,out.avg_C_conds,out.mean_rasters_T] = get_pop_vector_corr(out,conditionsAndRasterTypes,50,cellSel_A);
        all_out_A{ii} = out;
    end
    save('pop_corr_mat_vector_air_belt.mat','all_out_C','all_out_A','a_trials');
    disp('Done');
    return;
else
    temp = load('pop_corr_mat_vector_air_belt.mat');
...
mask = ones(size(allC{1})); mask = triu(mask,0)&~triu(mask,1);
...
MI = max(maxC);
for ii = 1:4
    axes(ff.h_axes(1,ii)); caxis([0 1]);
    axes(ff.h_axes(2,ii)); caxis([mI MI]);
%     axes(ff.h_axes(3,ii)); caxis([mIa maxs(3)]);
end

hc = putColorBar(ff.h_axes(1,4),[0.0 0.03 0 -0.05],[0 1],6,'eastoutside',[0.07 0.07 0.1 0.1]);
hc = putColorBar(ff.h_axes(2,4),[0.0 0.03 0 -0.05],[mI MI],6,'eastoutside',[0.07 0.07 0.1 0.1]);
% hc = putColorBar(ff.h_axes(3,4),[0.0 0.03 0 -0.05],[mIa maxs(3)],6,'eastoutside',[0.07 0.07 0.1 0.1]);

save_pdf(ff.hf,mData.pdf_folder,sprintf('figure_place_cells_py_10_1_%d_belt.pdf',cccc),600);
...
save_pdf(ff.hf,mData.pdf_folder,sprintf('figure_place_cells_py_10_2_%d_belt.pdf',cccc),600);
