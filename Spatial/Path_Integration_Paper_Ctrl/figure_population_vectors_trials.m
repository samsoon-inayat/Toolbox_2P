...
for ii = 1:length(a_trials)
    out = get_mean_rasters(pMs_C',paramMs_C,selAnimals_C,ei_C,conditionsAndRasterTypes',selC,'',a_trials{ii});
    [out.allP_an,out.allC_an,out.avg_C_conds] = get_pop_vector_corr(out,conditionsAndRasterTypes,min(out.sz(:)));
    all_out_C{ii} = out;
    out = get_mean_rasters(pMs_A',paramMs_A,selAnimals_A,ei_A,conditionsAndRasterTypes',selC,'',a_trials{ii});
    [out.allP_an,out.allC_an,out.avg_C_conds] = get_pop_vector_corr(out,conditionsAndRasterTypes,49);
    all_out_A{ii} = out;
end
...
all_out_C = pop_corr.all_out_C;
all_out_A = pop_corr.all_out_A;
