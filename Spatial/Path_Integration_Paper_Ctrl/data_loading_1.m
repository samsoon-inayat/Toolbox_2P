...
new_pre_pd_folder = '\\mohajerani-nas.uleth.ca\storage\homes\brendan.mcallister\2P\Processed_Data\OLD';

dir_list = dir(new_pre_pd_folder);
...
for ii = 1:size(T1,1)
    ...
    for jj = 1:length(dir_list)
        if ~dir_list(jj).isdir
            continue;
        end
        if strcmp(dir_list(jj),'.')
            continue;
        end
        if strcmp(dir_list(jj),'..')
            continue;
        end
        animal_id_C = str2num(dir_list(jj).name);
        if animal_id == animal_id_C
            found = 1;
            break;
        end
    end
    if found
        pd_folder = T1{ii,7}{1};
        ind_a = strfind(pd_folder,num2str(animal_id));
        ind_s = strfind(pd_folder(1:ind_a),'\');
        pd_folder = fullfile(new_pre_pd_folder,pd_folder((ind_s(end)+1):end));
        disp(pd_folder)
        T1{ii,7} = {pd_folder};
    end
end
