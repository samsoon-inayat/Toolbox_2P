...
for rr = 1:size(pMs_C,1)
    for cc = 1:size(pMs_C,2)
        ...
        for an = 1:length(selAnimals_C)
            zMIs_C(an,rr,cc) = nanmean(squeeze(pMs_C{rr,cc}.all_fFR{selAnimals_C(an)}(nds(1),nds(2),:)));
            a_zMIs_C{an,rr,cc} = squeeze(pMs_C{rr,cc}.all_fFR{selAnimals_C(an)}(nds(1),nds(2),:));
...
%     tcolors ={colors{1};colors{1};colors{2};colors{2};colors{3};colors{3};colors{4};colors{4}};
    ...
    xticks = xdata(1:end); xticklabels = {'C1-T','C2-T','C3-T','C4-T'};
    xtickangle(30)
