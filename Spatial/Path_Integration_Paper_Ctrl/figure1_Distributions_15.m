...
cellsOrNot = NaN; planeNumber = NaN; zMI_Th = 3; fwids = [1 120]; fcens = [0 145]; rs_th = 0.4; HaFD_th = NaN; HiFD_th = NaN;
cellsOrNot = NaN; planeNumber = NaN; zMI_Th = NaN; fwids = NaN; fcens = NaN; rs_th = NaN; HaFD_th = NaN; HiFD_th = NaN;
conditionsAndRasterTypes = [31 32 33]';
...
for rr = 1:size(pMs,1)
    for cc = 1:size(pMs,2)
        ...
        for an = 1:length(selAnimals)
            zMIs{an,rr,cc} = (squeeze(pMs{rr,cc}.all_zMIs{selAnimals(an)}(nds(1)-2,nds(2),:)));
...
if runthis
    ...
    hf = figure(1001);clf;set(gcf,'Units','Inches');set(gcf,'Position',[5 6 1.5 1],'color','w');
    hold on;
    ...
    xdata = 1:1.5:8; xdata = xdata(1:length(means));
