...
this_moas = get_sel_values(pa7,[1 3:11],selColsAll);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
...
this_moas = get_sel_values(motd,[1 3:11],selColsAll);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
...
put_axes_labels(gca,{[],[0 0 0]},{{'Trial Distance (cm)'},[0 0 0]});
...
this_moas = get_sel_values(mott,[1 3:11],selColsAll);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
...
this_moas = get_sel_values(moml,[1 3:11],selColsAll);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
