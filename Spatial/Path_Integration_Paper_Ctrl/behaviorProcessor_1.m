...
ii = 1;
moas = NaN(size(ei));
moasi = moas; mott = moas; motd = moas;
pa7 = moas;
for iii = 1:size(ei,1)
    if iii == 2
        n = 0;
...
runthis = 1;
if runthis
this_moas = moas(5:11,1:3); this_moas(2,:) = moas(6,[2 3 4]);
this_moasi = moasi(5:11,1:3); this_moasi(2,:) = moasi(6,[2 3 4]);
...
runthis = 1;
if runthis
this_moas = pa7(5:11,1:3); this_moas(2,:) = pa7(6,[2 3 4]);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
...
runthis = 1;
if runthis
this_moas = mott(5:11,1:3); this_moas(2,:) = mott(6,[2 3 4]);
for ii = 1:size(this_moas,2)
    varNames{ii} = sprintf('Trials_Day%d',ii);
end
data = [this_moas];
...
mtt = mas; mtd = mas; semtt = semas; semtd = semas;
...
%     set(gca,'FontSize',12,'FontWeight','Bold');
    [as{ii},tt{ii},td{ii}] = findAverageSpeedTrials(b);
...
out.mttT = mtt; out.mtdT = mtd; out.semttT = semtt; out.semtdT = semtd;
n = 0;
...
function [as,trial_time,trial_dist] = findAverageSpeedTrials (b)
if isfield(b,'stim_f')
    b.air_puff_r = b.stim_r;
    b.air_puff_f = b.stim_f;
end
