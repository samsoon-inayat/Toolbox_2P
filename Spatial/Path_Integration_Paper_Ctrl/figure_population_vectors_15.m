...
cellsOrNot = NaN; planeNumber = NaN; zMI_Th = 3; fwids = NaN; fcens = NaN; rs_th = 0.5; HaFD_th = NaN; HiFD_th = NaN;
...
for rr = 1:size(pMs,1)
    for cc = 1:size(pMs,2)
        ...
        for an = 1:length(selAnimals)
            zMIs(an,rr,cc) = nanmean(squeeze(pMs{rr,cc}.all_zMIs{selAnimals(an)}(nds(1)-2,nds(2),:)));
        end
    end
end
