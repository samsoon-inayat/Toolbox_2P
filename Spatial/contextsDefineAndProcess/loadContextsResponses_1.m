function ei = loadContextsResponses_1(ei,owr,owrp)
        ...
        for contextNumber = 1:length(contexts)
            ...
            for jj = 1:length(stimMarkers)
                ...
                for kk = 1:length(typesOfRasters)
                    ...
                    rasters = getRasters_fixed_bin_width_1(tei,pp,markersOn,markersOff,thisRasterType);
                    trials = 4:size(rasters.sp_rasters,1);
                    rasters = findRasterProperties_1(thispFolder,contextNumber,thisStimMarker,rasters,thisRasterType,trials,owrp);
