function ei = load_contexts_responses(ei,dc_filename,binWidths)
...
%         temp = load(fileName);
        if ~isfield(tplane,'contexts')
            contexts = getContexts({ptei},dc_filename);
        ...
        for contextNumber = 1:length(contexts)
            ...
            for jj = 1:length(stimMarkers)
                ...
                for kk = 1:length(typesOfRasters)
                    ...
                    rasters = getRasters_fixed_bin_width_new(tei,pp,markersOn,markersOff,thisRasterType,binWidths);
