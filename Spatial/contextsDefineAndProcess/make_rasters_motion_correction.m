function rasters =  make_rasters_motion_correction(ei,pp,onsets,offsets,rasterType,binwidths)
...
[xoff,yoff] = get_ca_motion_correction_data(ei,pp);
ccs = 1;
...
for trial = 1:length(onsets)
    ...
    for bin = 1:nbins
        ...
        distL1(trial,bin) = b.dist(st1(bin))-b.dist(st);
        distL2(trial,bin) = b.dist(se1(bin))-b.dist(st);
...
%             spSig = ei.deconv.spSigAll{cn}';
            spSig = ei.plane{pp}.tP.deconv.spSigAll(cn,:);
%             spSig = ei.deconv.spSigAll(cn,:);
            if ~isempty(frames)
