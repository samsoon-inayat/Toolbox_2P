...
for ii = 1:length(ei_C)
    ei_C(ii) = load_context_info(ei_C(ii),binwidths,[0 0 0],dcfilename);
    ei_C(ii) = load_motion_correction_empty(ei_C(ii),binwidths,[0 0 0]);
end
disp('Done');
%%
binwidths = [0.11 3];
for ii = 1:length(ei_A)
    ei_A(ii) = load_context_info(ei_A(ii),binwidths,[0 0 0],dcfilename);
    ei_A(ii) = load_motion_correction_empty(ei_A(ii),binwidths,[0 0 0]);
end
disp('Done');
%%
tic
...
for ii = 1:length(ei_C)
    ei_C(ii) = get_control_light_onsets(ei_C(ii));
    ei_C(ii) = get_control_air_onsets(ei_C(ii));
    ei_C(ii) = get_control_air_onsets_C(ei_C(ii));
    ei_C(ii) = get_control_air_offsets(ei_C(ii));
end

%% get control light onsets
for ii = 1:length(ei_A)
    ei_A(ii) = get_control_light_onsets(ei_A(ii));
    ei_A(ii) = get_control_air_onsets(ei_A(ii));
    ei_A(ii) = get_control_air_onsets_C(ei_A(ii));
    ei_A(ii) = get_control_air_offsets(ei_A(ii));
end
...
tic
for ii = 1:length(ei_C)
    ei_C(ii) = get_motion_onset_response(ei_C(ii),[0 0 0 0 0]);
end


for ii = 1:length(ei_A)
    ei_A(ii) = get_motion_onset_response(ei_A(ii),[0 0 0 0 0]);
end
