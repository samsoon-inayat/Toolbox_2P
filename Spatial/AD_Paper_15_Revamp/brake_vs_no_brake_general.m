...
while 1
    ...
    G = 'C'; all_cells_list = all_gV_C;
%     G = 'A'; all_cells_list = all_gV_A;
%     all_cells_list = all_exc_inh_C;
%     all_cells_list = all_gV_C;
    sh = 0;
...
%     plot([0 150.5],[(10.5+((ii-1)*10)) (10.5+((ii-1)*10))],'w','linewidth',0.1); 
    ...
    colormap jet
    save_pdf(hf,mData.pdf_folder,sprintf('OI_Map_%d_mean_%d_%c.pdf',ntrials,sh,G),600);
    ...
    box on
    colormap jet
    save_pdf(hf,mData.pdf_folder,sprintf('OI_Map_%d_sem_%d_%c.pdf',ntrials,sh,G),600);
...
%     delete(ff.h_axes(1,3));
    save_pdf(ff.hf,mData.pdf_folder,sprintf('OI_Map_cluster_%d_%c.pdf',sh,G),600);
