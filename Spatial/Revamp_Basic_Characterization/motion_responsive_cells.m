...
contexts = getContexts(ei,'define_contexts_1.m');
condition_number = 10;
b = ei{1}.b;
...
raster_data =  getRasters_fixed_bin_width_new(ei{1},pp,markersOn,markersOff,'time',[0.1,1.5]);
% raster_data_d =  getRasters_fixed_bin_width_ctrl(ei{1},pp,onsets,offsets,'dist');
rasters = raster_data.fromFrames.sp_rasters;
rasters = raster_data.sp_rasters1;
...
plotRasters_multi({rastersD1,rastersD2,rastersD3,rasters},find(zMIs3'>5),[])
return;
...
mdata.cis = cis(1,2:3);
plotRasters_simple(rasters,find(resp.ps(:,1)<0.01),mdata)
