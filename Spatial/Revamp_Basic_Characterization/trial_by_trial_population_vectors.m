...
while 1
    ...
    Rs = allRsC{cn}(an,:);mR = allmRsT{cn}(an,:);
    for cc = 1:size(mR,2)
        this_mat = mR{1,cc};
    end
    ff = makeFigureRowsCols(107,[1 0.5 4 0.5],'RowsCols',[2 10],...
        'spaceRowsCols',[0.1 0.05],'rightUpShifts',[0.03 0.13],'widthHeightAdjustment',...
        [-50 -250]);    set(gcf,'color','w');    set(gcf,'Position',[1 3 14 3]);
    [CRc,aCRc,mRR] = find_population_vector_corr(Rs,mR,respT,10);
...
%     ff = makeFigureRowsCols(108,[1 0.5 4 0.5],'RowsCols',[1 10],...
%         'spaceRowsCols',[0 0.05],'rightUpShifts',[0.03 0.13],'widthHeightAdjustment',...
%         [-50 -370]);    set(gcf,'color','w');    set(gcf,'Position',[1 8 14 1.6]);
%     ff = show_population_vector_and_corr(mData,ff,Rs,[],aCRc,[],[]);
...
%     colormap parula
%     save_pdf(ff.hf,mData.pdf_folder,sprintf('aPV_trials_%s.pdf',selected_property),600);
    %%
    break;
end
