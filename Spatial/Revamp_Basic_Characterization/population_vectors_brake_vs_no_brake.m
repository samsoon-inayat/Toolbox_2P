...
%% population vector and correlation sensory for time rasters with MC motion profile
    ...
    ff = makeFigureRowsCols(108,[6 3 2.1 3.1],'RowsCols',[4 2],'spaceRowsCols',[0.025 0.01],'rightUpShifts',[0.14 0.09],'widthHeightAdjustment',[10 -35]);
%     set(gcf,'color','w');    set(gcf,'Position',[10 3 3.5 3.75]);
    ...
    an = 1;G = 'C';
    si = [Ab_On Ar_On]; headstr = {'Brake (B)','No-Brake (NB)'}; headstrpos = [0 -0.14 0 0];
%     si = [Lb ArL_L]; headstr = {'Brake (B)','No-Brake (NB)'}; headstrpos = [0 -0.14 0 0];
%     si = [M_On M_Off]; headstr = {'Motion Onset (MOn)','Motion Offset (MOff)'}; headstrpos = [0 -0.14 0.05 0];
...
%         semspeed_over_trials = std(Rs{an,ii}.speed)/sqrt(10);
        ...
        changePosition(ha,[0 pos(4)-0.63 0 -0.5]);
        
        %******* make new axes and plot mean and gaussian fitting
        pos = get(ha,'Position'); ha = axes; set(ha,'units','inches');set(ha,'Position',pos);
        changePosition(ha,[0 pos(4)+0.11 0 0]);
        szln = size(Rs{an,ii}.speed,2)-1;
        mean_speed_over_trials = nanmean(Rs{an,ii}.speed(:,1:szln))*pixelSize(ii);
        semspeed_over_trials = std(Rs{an,ii}.speed(:,1:szln))/sqrt(10);
        xsMC = linspace(-2,2,size(mean_speed_over_trials,2));
        plot(xsMC,mean_speed_over_trials);
        shadedErrorBar(xsMC,mean_speed_over_trials,semspeed_over_trials);
