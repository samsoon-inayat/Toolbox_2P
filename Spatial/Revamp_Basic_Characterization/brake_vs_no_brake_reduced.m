...
while 1
    ...
    rasterNamesTxt = {'1-L','4-L','6-L','2-AOn','7-AOn','2-AOff','7-AOff',...
        ...
        '3-Lc','5-Lc','2-AOffc','3-AOffc','4-AOffc','5-AOffc','7-AOffc',...
...
%     h(h==1) = 0;
    ...
    format_axes(gca);
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    set_axes_limits(gca,[0.35 xdata(end)+.65],[ylims(1) 20]); format_axes(gca);
    xticks = xdata; xticklabels = {'B-Exc','NB-Exc','B-Inh','NB-Inh'};
...
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    set_axes_limits(gca,[0.35 xdata(end)+.65],[ylims(1) 20]); format_axes(gca);
    xticks = xdata; xticklabels = {'B-Exc','NB-Exc','B-Sup','NB-Sup'};
    set(gca,'xtick',xticks,'xticklabels',xticklabels,'ytick',[0 5 10 15]); xtickangle(45)
    changePosition(gca,[0.05 0.01 -0.5 -0.04]); %put_axes_labels(gca,{[],[0 0 0]},{{'Responsive','Cells (%)'},[0 0 0]});
...
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    set_axes_limits(gca,[0.35 xdata(end)+.65],[ylims(1) 60]); format_axes(gca);
    xticks = xdata; xticklabels = {'B-Exc','NB-Inh','B-Sup','NB-Sup'};
    set(gca,'xtick',xticks,'xticklabels',xticklabels,'ytick',[0 25 50]); xtickangle(45)
    changePosition(gca,[0.05 0.01 -0.5 -0.04]); %put_axes_labels(gca,{[],[0 0 0]},{{'Responsive','Cells (%)'},[0 0 0]});
...
while 1
    ...
    event_type = {'Air ON','Air OFF','Light ON'};
%     sic = {[Lb Lbs Ab_On Abs_On];[Ab_Off Abs_Off];ArL_L;[Ar_On ArL_On Ars_On];[Ar_Off ArL_Off Ars_Off]};%;M_On;M_Off};
    allsic = {{[Ab_On Abs_On];[Ar_On ArL_On Ars_On];[Ab_Off Abs_Off];[Ar_Off ArL_Off Ars_Off];[Ab_Offc Abs_Offc];[Ar_Offc ArL_Offc Ars_Offc]}};
%     ind = 3;
%     sic = allsic{1:6};
    all_resp = []; all_resp_exc = []; all_resp_inh = [];
    all_respV = [];
    ...
    [within,dvn,xlabels] = make_within_table({'Type','ST','Cond'},[2,3,2]);
    dataT = make_between_table({per_active},dvn);
    ra = RMA(dataT,within);
    ra.ranova
    print_for_manuscript(ra)
    ...
    [mrall,semrall] = findMeanAndStandardError(per_active_all);
    %%
    per_all_respV = find_percent(all_respV);
     [within,dvn,xlabels] = make_within_table({'ST','Cond'},[3,2]);
    dataT = make_between_table({per_all_respV},dvn);
    ra = RMA(dataT,within);
    ra.ranova
    print_for_manuscript(ra)
  %%
...
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    ...
    %%
    [xdata,mVar,semVar,combs,p,h,colors,xlabels] = get_vals_for_bar_graph_RMA(mData,ra,{'ST_by_Cond','hsd'},[1.5 1 1]);
    h(h==1) = 0;
