...
%%
if 1
% for testing spatial correlation of excitatory and inhibitory cells
...
between = make_between_table({varE1,varE2;varI1,varI2},dvn);
...
if 1
    ...
    [all_corr_CAE,all_corr_cell_CAE,mean_corr_CAE,mean_cell_corr_CAE,xs_CAE,paramAE] = find_population_vector_corr_remap(Rs,mR,respE_OR1);
    ...
    [all_corr_CAI,all_corr_cell_CAI,mean_corr_CAI,mean_cell_corr_CAI,xs_CAI,paramAI] = find_population_vector_corr_remap(Rs,mR,respI_OR1);
...
%%
if 1
    mean_cell_corr_C = outE.adj_SP_corr_diag;
    for ii = 1:size(mean_cell_corr_C,1)
        var_CE(ii,1) = mean(mean_cell_corr_C{ii});
    end
    
    mean_cell_corr_C = outI.adj_SP_corr_diag;
    for ii = 1:size(mean_cell_corr_C,1)
        var_CI(ii,1) = mean(mean_cell_corr_C{ii});
    end
...
%     xtickangle(30);
%     changePosition(gca,[0.2 0.03 -0.55 -0.05]);
    changePosition(gca,[0.31 0.03 -0.4 -0.05]);
    put_axes_labels(gca,{[],[0 0 0]},{{'Pearson''s','Correlation'},[0 0 0]});
