...
oanimal = get_exp_info(mD,animal_list,date_list);
...
animal = get_exp_info(mD1,animal_list_1,date_list_1);
...
%% Get Video Pointers
for ii = 1:4
    vpa{ii} = VideoReader(animal(ii).video.mp4.paws);
    vfa{ii} = VideoReader(animal(ii).video.mp4.face);
    va{ii} = VideoReader(animal(ii).video.mp4.pupil);
end
