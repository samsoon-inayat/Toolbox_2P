...
%     end
    if mod(sii,2) ~= 0
        text(3,size(P,1)+round(size(P,1)/10),sprintf('Pre - %s',exih{sii}),'FontSize',FS+1,'FontWeight','Normal');
    else
    ...
    ts = linspace(-1,1,50);%(adata{selAnimals(1)}{1}{1}.duration(1,:));
...
text(xlims(1)+0.39,ylims(2)+0.045,'1','FontSize',4.5);
save_pdf(ff.hf,mData.pdf_folder,'figure_air_cells_py_15.pdf',600);
