...
%         excRAll{jj,ii} = tempD(1,:);
%         excR(jj,ii) = sum(tempD(1,:));
%         [tempD cnsjj] = getVariableValues(tdata{selAnimals(jj)},'inhR',ii);
%         inhRAll{jj,ii} = tempD(1,:);
%         inhR(jj,ii) = sum(tempD(1,:));
%     end
end
distD = distD5;
for jj = 1:length(selAnimals)
...
% set(gca,'XTick',[1:11],'XTickLabel',{'5','7','7','9','9','8','8','6','6','10','10'});
