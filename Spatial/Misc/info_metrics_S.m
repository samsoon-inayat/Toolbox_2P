function [output FRbin] = info_metrics_S( FR, stimulus, nbinFRSH, occupSK, nshuffles)
...
output.corr = corr(stimulus, FRtemp);
% return;
...
if nshuffles>0
    ...
    for s=1:nshuffles
        shuffleoutput = info_metrics_S( shuffle(FRtemp), stimulus, nbinFRSH, occupSK, 0); %recursive
        output.ShannonMI_shuffle(s)   = shuffleoutput.ShannonMI;
        output.corr_shuffle(s) = corr(stimulus,shuffle(FRtemp));
    ...
    mu=mean(output.corr_shuffle); si=std(output.corr_shuffle);
    output.corr_p = sum(output.corr>output.corr_shuffle)/nshuffles;
    output.corr_Zsh=(output.corr-mu)/si;
