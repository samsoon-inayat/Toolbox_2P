...
processed_data_folder{2} = '\\mohajerani-nas.uleth.ca\storage\homes\brendan.mcallister\2P\Processed_Data_15\Matlab11';
processed_data_folder{3} = '\\mohajerani-nas.uleth.ca\storage\homes\brendan.mcallister\2P\Processed_Data_15\Matlab_bw311';
...
binwidths = [0.11 3];
for ii = 1:length(ei)
    ei(ii) = load_context_info(ei(ii),binwidths,[0 0 0],'define_cont_11.m');
end
disp('Done');
%%
tic
...
tic
for ii = 1:length(ei)
    ei(ii) = make_and_load_rasters_11(ei(ii),binwidths,[0 0 0]);
end
toc
%%
binwidths = [0.11 3];
for ii = 1:length(ei)
    ei(ii) = make_and_load_motion_correction_11(ei(ii),binwidths,[0 0 0]);
end
