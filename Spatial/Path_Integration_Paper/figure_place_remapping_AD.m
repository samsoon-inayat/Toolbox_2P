...
cellsOrNot = 1; planeNumber = NaN; zMI_Th = 1.96; fwids = [1 120]; fcens = [0 140]; rs_th = 0.3;FR = NaN;
% cellsOrNot = NaN; planeNumber = NaN; zMI_Th = 3; fwids = [1 120]; fcens = [0 140]; rs_th = 0.4; HaFD_th = NaN; HiFD_th = NaN;
andor = 0;
if andor == 1
    conditionsAndRasterTypes = [11 21 31 41]; % and combination to find common cells across conditions
else
    conditionsAndRasterTypes = [11;21;31;41]; % and combination to find common cells across conditions
end
...
filename_and = fullfile(mData.pd_folder,'pop_corr_mat_remapping_and_AD.mat');
filename_or = fullfile(mData.pd_folder,'pop_corr_mat_remapping_or_AD.mat');
if 0
    ...
    if andor == 1
        save(filename_and,'all_out_C','all_out_A','a_trials');
    else
        save(filename_or,'all_out_C','all_out_A','a_trials');
    end
    disp('Done');
    return;
else
    temp = load(filename_and);
