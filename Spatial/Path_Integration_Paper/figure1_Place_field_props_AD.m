...
ylabels = {'zMIs','Firing Rate','PF Widths','RS'};
maxYs = [10,100,60,0.7];
svn = 3; gcn = 3
...
if runthis
%     cN = 1;
    data_C = squeeze(out_C.all_data_N(gcn,:,:))'
    data_A = squeeze(out_A.all_data_N(gcn,:,:))'
    data_C = squeeze(out_C.all_data(gcn,:,:))';
    data_A = squeeze(out_A.all_data(gcn,:,:))';
...
%     text(18.5,18,'APP','FontSize',7);
    % applyhatch_plusC(gcf
    save_pdf(hf,mData.pdf_folder,sprintf('%s_distributions',mfilename),600);
return;
end
