...
ra = repeatedMeasuresAnova(data,within);
n = 0;
%%
if 1
    ...
    xdata = [1 2 3 4 5 6 [1 2 3 4 5 6]+7]; 

    hf = figure(5);clf;set(gcf,'Units','Inches');set(gcf,'Position',[5 7 2.25 1.5],'color','w');
    hold on;
    tcolors = {colors{1};colors{2};colors{3};colors{4};colors{5};colors{6}};
    tcolors = repmat(tcolors,2,1)';
    [hbs,maxY] = plotBarsWithSigLines(mVar,semVar,combs,[h p],'colors',tcolors,'sigColor','k',...
        'ySpacing',5,'sigTestName','','sigLineWidth',0.25,'BaseValue',0.01,...
        'xdata',xdata,'sigFontSize',7,'sigAsteriskFontSize',8,'barWidth',0.5,'sigLinesStartYFactor',0.01);

    set(gca,'xlim',[0.25 13.75],'ylim',[0 maxY-1],'FontSize',6,'FontWeight','Bold','TickDir','out');
    xticks = [1.5 3.5 5.5 8.5 10.5 12.5]; xticklabels = {'Day1','Day2','Day3'}; xticklabels = repmat(xticklabels,2,1)';
...
if 1
    ...
    hf = figure(5);clf;set(gcf,'Units','Inches');set(gcf,'Position',[5 7 1.25 1.5],'color','w');
    hold on;
    tcolors = {colors{1};colors{2};colors{3};colors{4};colors{5};colors{6}};
    tcolors = repmat(tcolors,2,1)';
    [hbs,maxY] = plotBarsWithSigLines(mVar,semVar,combs,[h p],'colors',tcolors,'sigColor','k',...
    ...
    set(gca,'xlim',[0.25 6.75],'ylim',[0 maxY-1],'FontSize',6,'FontWeight','Bold','TickDir','out');
    xticks = [1.5 3.5 5.5]; xticklabels = {'Day1','Day2','Day3'}; 
    set(gca,'xtick',xticks,'xticklabels',xticklabels);
    xtickangle(30);
%     for ii = 7:length(hbs)
%         set(hbs(ii),'facecolor','none','edgecolor',tcolors{ii});
