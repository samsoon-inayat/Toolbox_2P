function [speedRs] = load_speed_responseZ(ei)%[speedRs,resp_speed,pR,resp_speedN] = load_speed_responseZ(ei)
...
for ii = 1:length(ei)
    tei = ei{ii};
    psp = [];
    psp1 = tei.plane{1}.speed_responseZ;
    if length(tei.plane) == 2
        % psp2 = tei.plane{2}.speed_responseZ;
...
for ii = 1:length(ei)
    tei = ei{ii};
    psp = [];
    psp1 = tei.plane{1}.accel_responseZ;
    if length(tei.plane) == 2
