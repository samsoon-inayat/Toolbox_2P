function air_vs_belt

%% general for all properties including responsivity, response fidelity, zMI, Rs
while 1
    ntrials = 50; 
    si = [Ar_t_T ArL_t_T Ars_t_T Ar_t_D ArL_t_D Ars_t_D Ar_i_T ArL_i_T Ars_i_T Ar_i_D ArL_i_D Ars_i_D Ar_B_T ArL_B_T Ars_B_T Ar_B_D ArL_B_D Ars_B_D];
%     si = [C1_i_T C2_i_T C3_i_T C4_i_T];
    ...
    varC = mean_var_C;
    [within,dvn,xlabels,awithinD] = make_within_table({'TI','DT','Cond'},[3,2,3]);
...
stp = 0.225*magfac; widths = ([1.75 0.75 1.3 1.3 1.3 0.5 0.5 0.5]-0.05)*magfac; gap = 0.067*magfac;
...
make_bars_hollow(hbs(7:12));
[~,hyl] = put_axes_labels(gca,{'',[]},{ylabeltxt,[]}); set(hyl,'FontWeight','bold');
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,2,{'C3','C4','C5','C3','C4','C5','C3','C4','C5'},{[0 0.03]});
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,6,{'Trials','InterTrials','Belt'},{[-0.1 -0.012]});
...
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,6,{'Pooled'},{[0 0]});
format_axes(gca);
save_pdf(ff.hf,mData.pdf_folder,'bar_graph.pdf',600);

%% RF two graphs, all and cond
ff = makeFigureRowsCols(107,[10 5 2.25 1.25],'RowsCols',[1 1],'spaceRowsCols',[0.01 -0.02],'rightUpShifts',[0.07 0.36],'widthHeightAdjustment',[10 -510]);
switch varT
    case 1 % responsive cells 
        MY = 70; ysp = 4; mY = 0; titletxt = 'Responsivity'; ylabeltxt = {'% of cells'}; % for all cells (vals) MY = 80
    case 2
...
% set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,2,{'Trials','InterTrials'},{[-0.1 -0.012]});
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,6,{'Pooled'},{[0 0]});
ht = set_axes_top_text_no_line(gcf,gca,titletxt,[-0.05 -0.051 0.2 0]);set(ht,'FontWeight','Bold');
format_axes(gca);
save_pdf(ff.hf,mData.pdf_folder,'bar_graph.pdf',600);
...
switch varT
    ...
    case 3
        MY = 0.1; ysp = 0.01; mY = -0.15; titletxt = 'Mutual Information'; ylabeltxt = {'Mutual Information','Z-Score'};
        MY = 6.5; ysp = 0.3; mY = 0; titletxt = 'Mutual Information'; ylabeltxt = {'(z-score)'};
    case 9
        MY = 20; ysp = 1; mY = -0.15; titletxt = 'Tuning Width'; ylabeltxt = {'cm'};
end
stp = 0.225*magfac; widths = ([0.55 0.55 1.3 1.3 1.3 0.5 0.5 0.5]-0.05)*magfac; gap = 0.067*magfac;
...
RsDb = o.Rs(:,[Ar_B_D ArL_B_D Ars_B_D]);  RsTb = o.Rs(:,[Ar_B_T ArL_B_T Ars_B_T]);
dzMI_t = get_dzMI_based_dist_time(RsDt,RsTt); 
dzMI_i = get_dzMI_based_dist_time(RsDi,RsTi);
dzMI_b = get_dzMI_based_dist_time(RsDb,RsTb);


%%
sel_pop_C = [dzMI_t.resp_T_g_D dzMI_t.resp_D_g_T dzMI_i.resp_T_g_D dzMI_i.resp_D_g_T dzMI_b.resp_T_g_D dzMI_b.resp_D_g_T];
