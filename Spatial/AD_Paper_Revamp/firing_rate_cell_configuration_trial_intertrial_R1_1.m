...
magfac = mData.magfac;
ff = makeFigureRowsCols(108,[4 5 2.3 1.0],'RowsCols',[1 3],'spaceRowsCols',[0.03 -0.01],'rightUpShifts',[0.1 0.25],'widthHeightAdjustment',[10 -500]);
MY = 8; ysp = 1; mY = 0; 
stp = 0.3*magfac; widths = ([0.5 0.5 0.5 0.5 0.25 0.25 0.25 0.25]+0.05)*magfac; gap = 0.16*magfac;
...
%         put_axes_labels(ha,{'Firing Rate (A.U.)',[0 0 0]},{{'Neurons (%)'},[0 0 0]});
    ...
    if varT == 3
        titletxt = sprintf('n = %d,',length(allValsG{1}));
        ht = set_axes_top_text_no_line(gcf,ha,titletxt,[0.02 -0.3 0 0]);set(ht,'FontSize',6,'Color','k');
        titletxt = sprintf('%d',length(allValsG{2}));
        ht = set_axes_top_text_no_line(gcf,ha,titletxt,[0.05 -0.4 0 0]);set(ht,'FontSize',6,'Color','r');
    end
end
