...
for rr = 1:size(Rs,1)
    for cc = 1:size(Rs,2)
        ...
        if strcmp(R.marker_name,'motionOnsets') || strcmp(R.marker_name,'motionOffsets')
            [Rs{rr,cc}.resp.vals,Rs{rr,cc}.resp.cis] = find_resp_motionOnset_raster(R,trials);
        end
...
%             Rs{rr,cc}.resp.vals = R.iscell' & zMIs > 1.96 & rs > 0.25 & PWs < 150 & centers > 0 & centers < 150;
            Rs{rr,cc}.resp.vals = R.iscell' & zMIs > 1.65 & rs > 0.25 & PWs < 150 & centers > 0 & centers < 150 & MFR < 10000;
        end
