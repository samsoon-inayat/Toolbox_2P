...
%             [Rs{rr,cc}.resp.vals,Rs{rr,cc}.resp.cis] = find_resp_time_raster_light_fractal(R,trials);
        ...
        if strcmp(R.marker_name,'airT') || strcmp(R.marker_name,'beltT') || strcmp(R.marker_name,'airIRT') || strcmp(R.marker_name,'airRT')
            ...
            [Rs{rr,cc}.resp.vals,Rs{rr,cc}.resp.fac] = find_resp_no_before_after_anova(R,1:10,ar_owr(1)); 
            [Rs{rr,cc}.resp.valsT,Rs{rr,cc}.resp.facT] = find_resp_no_before_after_anovaT(R,1:10,ar_owr(2));
            [Rs{rr,cc}.resp.valsKW,Rs{rr,cc}.resp.facKW] = find_resp_no_before_after_KW(R,1:10,ar_owr(3));
            [Rs{rr,cc}.resp.valsKWT,Rs{rr,cc}.resp.facKWT] = find_resp_no_before_after_KWT(R,1:10,ar_owr(4));
%             [Rs{rr,cc}.resp.valsFT,Rs{rr,cc}.resp.facFT] = find_resp_no_before_after_FT(R,1:10);
%             [Rs{rr,cc}.resp.valsRM,Rs{rr,cc}.resp.facRM] = find_resp_no_before_after_anovaRM(R,1:10);
...
%             [rs1,coeffs] = getMRFS_vals(R.gauss_fit_on_mean);
            ...
            zMIs = R.info_metrics.ShannonMI_Zsh;
%             Rs{rr,cc}.resp.vals = R.iscell' & zMIs > 1.96 & rs > 0.25 & PWs < 150 & centers > 0 & centers < 150;
            Rs{rr,cc}.resp.valsA = R.iscell' & zMIs > 1.65 & rs > 0.25 & PWs > 1 & PWs < 150 & centers > 0 & centers < 150 & MFR < 10000;
...
function [resp,fac] = find_resp_no_before_after_anova(R,trials,owr)
% SR = R.thorexp.frameRate;
...
function [resp,fac] = find_resp_no_before_after_anovaT(R,trials,owr)
% SR = R.thorexp.frameRate;
file_name = fullfile(R.pd_folder,sprintf('responsive_cells_pyramidal_anovaT_%s.mat',R.context_info));
if exist(file_name,'file') && owr == 0
    te = load(file_name);
    resp = te.resp;
    resp = te.p<0.05;
...
function [resp,fac] = find_resp_no_before_after_KW(R,trials,owr)
% SR = R.thorexp.frameRate;
...
function [resp,fac] = find_resp_no_before_after_KWT(R,trials,owr)
% SR = R.thorexp.frameRate;
file_name = fullfile(R.pd_folder,sprintf('responsive_cells_pyramidal_KWT_%s.mat',R.context_info));
if exist(file_name,'file') && owr == 0
    te = load(file_name);
    resp = te.resp;
    resp = te.p<0.05;
...
function [resp,fac] = find_resp_no_before_after_anovaRM(R,trials,owr)
% SR = R.thorexp.frameRate;
file_name = fullfile(R.pd_folder,sprintf('responsive_cells_pyramidal_anovaRM_%s.mat',R.context_info));
if exist(file_name,'file') && owr == 0
    te = load(file_name);
    resp = te.resp;
    resp = te.p<0.05;
...
for ni = 1:length(fac)
    group = groups{ni};
    thisRaster = rasters(trials,:,1);
    [within,dvn,xlabels] = make_within_table({'bins'},[size(thisRaster,2)]);
    within.bins = group';
    parfor ii = 1:size(rasters,3)
        if [ni ii] == [1 31]
            n = 0;
        ...
        try
            ...
            p(ii,ni) = ra.ranova{3,ra.selected_pval_col};
        catch
            if sum(isnan(thisRaster(:))) > 0
                thisRaster = fillmissing(thisRaster,'linear',2,'EndValues','nearest');
                dataT = make_between_table({thisRaster},dvn);
...
function [resp,fac] = find_resp_no_before_after_FT(R,trials,owr)
% SR = R.thorexp.frameRate;
file_name = fullfile(R.pd_folder,sprintf('responsive_cells_pyramidal_FT_%s.mat',R.context_info));
if exist(file_name,'file') && owr == 0
    te = load(file_name);
    resp = te.resp;
    resp = te.p<0.05;
...
for ni = 1:length(fac)
    ...
    for ii = 1:size(rasters,3)
        thisRaster = rasters(trials,:,ii);
%         thisRasterG = make_gauss_fit_raster(R,ii);
        p(ii,ni) = friedman(thisRaster,group,'off');
%         pG(ii,ni) = anova1(thisRasterG,group,'nodisplay');
    end
end
resp = sum(p<0.05,2)>0;
