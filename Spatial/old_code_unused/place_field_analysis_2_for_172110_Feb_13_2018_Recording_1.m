function place_field_analysis_2_for_172110_Feb_13_2018_Recording_1
...
% plotMarkers(ei,b.air_puff_r,[],100);
A_Trials = b.trials(1:11);
...
photo_sensor = b.photo_sensor_f(b.photo_sensor_f>ap_a_offsets(end));
diff_photo_sensor = diff(photo_sensor);
inds = find(diff_photo_sensor < 100000);
temp_photo_sensor = photo_sensor;
temp_photo_sensor(inds) = [];

p_onsets = temp_photo_sensor(1:(length(temp_photo_sensor)-1));
p_offsets = temp_photo_sensor(2:end);


allA_P = getDistRaster_2(ei,a_p_onsets,a_p_offsets,0,0);
allAP_P = getDistRaster_2(ei,ap_p_onsets,ap_p_offsets,0,0); 
% allP = getDistRaster_2(ei,p_onsets,p_offsets,0,0);

for ii = 1:length(ccs)
    dSIs(ii) = allAP_P(ii).SI - allA_P(ii).SI;
end
figure(100);clf;histogram(dSIs,'Normalization','probability');
...
%         P = getDistRaster_1(b,caSig,tsp,p_onsets,p_offsets,0); P_raster = P.distSigRaster./P.distDurRaster;
%         P_SI = spatial_information(nanmean(P_raster),nanmean(P.distDurRaster));
...
%             if cc == 3
%                 plotDistRaster(ff.h_axes(rr,cc),P,[minD maxD],sprintf('%s - %.2f',num2str(cn),P_SI));
%             end
%             if cc == 4
